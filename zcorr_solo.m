...
clear all
%rootdir is full path to root directory of your tiff stacks
...
if SAVEIMG == 1
    blocksize = 2; %break up data processing into groups of trials this long
    xpix = 512;
    ypix = 512;
    cd(rootdir);
    FullList = dir;
    ...
    for block = 1:length(ALLBLOCKS)
        ...
        saveastiff(block2data(1:xpix,1:ypix, ALLBLOCKS(block).include), ['block_', num2str(block,'%02i') '_Ch2.tif']);
        saveastiff(block1data(1:xpix,1:ypix, ALLBLOCKS(block).include), ['block_', num2str(block,'%02i') '_Ch1.tif']);
    end    
end